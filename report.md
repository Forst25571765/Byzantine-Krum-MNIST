...
实验验证了 Krum 算法在拜占庭容错方面的优越性。只要满足 $n \gt 2f + 2$ 的条件，Krum 就能在不依赖历史信息或者多次通信的情况下，通过纯几何的方法筛选出可靠的梯度方向。

### 5.2 局限性分析

1. **收敛至“盆地”**：正如收敛性分析所述，Krum 最终只能收敛到一个梯度范数小于某个阈值的区域（盆地）。当梯度信号小于攻击者引入的噪声时，下降可能会停止。
