...
- **理论基础**：在 $n \gt 2f + 2$ 的条件下（即诚实节点占绝大多数），Krum 是首个被证明具有 **Byzantine-Resilient** 特性的算法。它能保证输出的更新向量在数学期望上与真实梯度方向一致且高阶矩有界。
...
1. **Omniscient (全知攻击/反向攻击)**：恶意节点将计算出的真实梯度进行反向并放大（乘以 $-100$），试图将模型推向反方向并放大噪声。
...
- **Avg (平均聚合)**：在 $f=3$ 的攻击下，Accuracy 迅速跌至 10% 左右，即随机猜测水平。这是因为恶意节点的 $-100$ 倍梯度直接淹没了 7 个诚实节点的梯度，模型参数发散。
...
高斯攻击虽然方向随机，但由于方差极大，平均聚合依然被严重干扰导致准确率下降，无法收敛。Krum 同样表现出极强的鲁棒性，能够收敛。
...
1. **收敛至“盆地”**：正如 Sec5. 收敛性分析所述，Krum 最终只能收敛到一个梯度范数小于某个阈值的区域（盆地）。当梯度信号小于攻击者引入的噪声时，下降可能会停止。这是拜占庭节点引入的不确定性所致。
